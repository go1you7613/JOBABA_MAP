...
--      (PUB-xxx / JK-xxx / JKE-xxx / JKR-xxx)
...
--   6. 운영 원천 테이블에는 INSERT/UPDATE/DELETE/DROP/TRUNCATE/RENAME을 실행하지 않음
--   7. staging/old 교체는 MAP 소유 파생 객체인 v_job_posting에만 사용
--   8. 기존 원천 수집 배치 완료 후 후속 단계에서 이 SQL을 실행
-- ============================================================
...
-- 운영 반영 전 db/work24_public_job_contract.sql의 계약을 확인하고,
-- 개발팀의 기존 고용24 공공기관 목록 배치가 복합 식별자를
-- tb_work24_public_job에 적재하도록 연결하십시오.
...
CREATE TABLE jwrki.v_job_posting_staging AS
SELECT * FROM jwrki.v_job_posting_source;

DROP VIEW jwrki.v_job_posting_source;

ALTER TABLE jwrki.v_job_posting_staging
    CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

CREATE INDEX idx_v_job_posting_auth_no ON jwrki.v_job_posting_staging (WANTED_AUTH_NO);
CREATE INDEX idx_v_job_posting_reg_dt ON jwrki.v_job_posting_staging (REG_DT);
CREATE INDEX idx_v_job_posting_close_dt ON jwrki.v_job_posting_staging (CLOSE_DT, REG_DT);
CREATE INDEX idx_v_job_posting_source_type ON jwrki.v_job_posting_staging (SOURCE_TYPE);
CREATE INDEX idx_v_job_posting_use_del ON jwrki.v_job_posting_staging (USE_YN, DEL_YN);
CREATE INDEX idx_v_job_posting_list
    ON jwrki.v_job_posting_staging (USE_YN, DEL_YN, SOURCE_TYPE, REG_DT);
CREATE INDEX idx_v_job_posting_pub_filters
    ON jwrki.v_job_posting_staging (
        JOBABA_CMMN_276_CD(3),
        JOB_CAREER_CD(10),
        JOB_ACDMCR_CD(10),
        JOB_EMP_TP_CD(10)
    );
CREATE INDEX idx_v_job_posting_prv_filters
    ON jwrki.v_job_posting_staging (
        JOBABA_CMMN_274_CD(1),
        JOB_CAREER_CD(10),
        JOB_ACDMCR_CD(10),
        JOB_EMP_TP_CD(10)
    );

-- 데이터 품질 검증과 최종 교체는 scripts/sync_v_job_posting.sh가 수행합니다.
-- 이 지점에서는 기존 v_job_posting을 변경하지 않습니다.
